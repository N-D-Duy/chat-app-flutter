...
class CallRemoteDataSourceImpl extends CallRemoteDataSource {
  ...
  @override
  Future<void> endCall(
      {required String callerId, required String receiverId}) async {
    await _firestore
        .collection('call')
        .doc(callerId)
        .collection('calling')
  ...
  Future<void> makeCall(BuildContext context,
    ...
    try {
      ...
      CallModel senderCallData = CallModel(
          ...
          callerPic: currentUser.profileImage,
      ...
      _saveToCallingSubCollection(
          senderCallData: senderCallData, receiverCallData: receiverCallData);

      _saveToHistorySubCollection(
          senderCallData: senderCallData, receiverCallData: receiverCallData);
  ...
  void _saveToCallingSubCollection(
      {required CallModel senderCallData,
      required CallModel receiverCallData}) async {
    await _firestore
        .collection('call')
  ...
  void _saveToHistorySubCollection(
      {required CallModel senderCallData,
      required CallModel receiverCallData}) async {
    await _firestore
        .collection('call')
